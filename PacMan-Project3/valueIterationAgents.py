...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for justcount in range(0, self.iterations):
        	ValueDict = util.Counter()
        	for eachstate in self.mdp.getStates():
        		if self.mdp.isTerminal(eachstate):
        			ValueDict[eachstate] = 0
        		else:
        			value = float("-inf")
        			Qvalues = []
              		for eachaction in self.mdp.getPossibleActions(eachstate):
              			Qvalues.append(self.computeQValueFromValues(eachstate, eachaction))
				ValueDict[eachstate] = max(value, max(Qvalues))
    ...
    def computeActionFromValues(self, state):
        ...
        if self.mdp.isTerminal(state):
        	...
        	for eachaction in self.mdp.getPossibleActions(state):
        		actionva = self.computeQValueFromValues(state, eachaction)
        		if actionva >= value:
        			value = actionva
        			policy = eachaction
        	return policy
