...
def enhancedFeatureExtractorDigit(datum):
    ...
    def Surrounding(coord):
        surr = []
        if coord[0] > 0:
            surr.append((coord[0] - 1, coord[1]))
        if coord[0] < DIGIT_DATUM_WIDTH - 1:
            surr.append((coord[0] + 1, coord[1]))
        if coord[1] > 0:
            surr.append((coord[0], coord[1] - 1))
        if coord[1] < DIGIT_DATUM_HEIGHT - 1:
            surr.append((coord[0], coord[1] + 1))
        return surr 
    count = 0
    region = []
            ...
            if (x, y) in region:
                ...
                if pix <= 1:
                    ...
                    while len(temp) != 0:
                        point = temp.pop()
                        region.append(point)
                        for eachpixel in Surrounding(point):
                            pix2 = datum.getPixel(eachpixel[0], eachpixel[1])
                            if pix2 < 2:
                                if eachpixel in region:
                                    continue
                                else:
                                    temp.append(eachpixel)
    features["CR1"] = count
    features["CR2"] = count % 2
    features["CR3"] = (count + 1) % 2
    for x in range(0, DIGIT_DATUM_WIDTH):
        for y in range(0, DIGIT_DATUM_HEIGHT):
...
def enhancedPacmanFeatures(state, action):
    ...
    "*** YOUR CODE HERE ***"
    successor = state.generateSuccessor(0, action)
    pacmanpositions = successor.getPacmanPosition()
    ghosts = successor.getGhostPositions()
    capsules = successor.getCapsules()
    sfood = state.getFood()
    food = [(x, y) for x, row in enumerate(sfood) for y, food in enumerate(row) if food]
    nearbycapsules = []
    for eachcapsule in capsules:
        distance = util.manhattanDistance(pacmanpositions, eachcapsule)
        nearbycapsules.append(distance)
    nearbycapsules.sort()
    for i in range(min(len(nearbycapsules), 1)):
        features[("capsule", i)] = 15 / (1 + nearbycapsules[i])
    nearghosts = []
    for eachghost in ghosts:
        distance = util.manhattanDistance(pacmanpositions, eachghost)
        nearghosts.append(distance)
    features["nearest_ghost"] = min(nearghosts)
    nearghosts.sort()
    for i in range(min(len(nearghosts), 1)):
        features[("ghost", i)] = 5 / (0.1 + nearghosts[i])
    nearfood = []
    for eachfood in food:
        distance = util.manhattanDistance(pacmanpositions, eachfood)
        nearfood.append(distance)
    nearfood.sort()
    for i, weights in zip(range(min(len(nearfood), 5)), [1.3, 0.8] + [0.9] * 10):
        features[("food", i)] = weights * nearfood[i]
    features["score"] = state.getScore() * 15
    features["capsule count"] = len(capsules) * 25
