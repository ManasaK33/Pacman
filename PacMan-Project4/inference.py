...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for loc in legal_positions:
                manh_dist = util.manhattanDistance(loc, pacmanPosition)
                distance = manh_dist
                allPossible[loc] = self.beliefs[loc] * emissionModel[distance]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        legal_positions = self.legalPositions
        for prev_loc in legal_positions:
            ghost_pos = self.setGhostPosition(gameState, prev_loc)
            new_distribution = self.getPositionDistribution(ghost_pos)
            for new_pos, p in new_distribution.items():
                z = p * self.beliefs[prev_loc]
                allPossible[new_pos] = allPossible[new_pos] + z
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        count = 0
        legal_positions = self.legalPositions
        no_of_Particles = self.numParticles
        while count < no_of_Particles:
            for position in legal_positions:
                if count < no_of_Particles:
    ...
    def observe(self, observation, gameState):
        ...
        pos = self.getJailPosition()
        if noisyDistance == None:
            self.particles =  [pos] * no_of_particles
        else:
            weight_zero = True
            weights = util.Counter()
            for particle in self.particles:
                manh_dist = util.manhattanDistance(particle, pacmanPosition)
                distance = manh_dist
                weight = emissionModel[distance]
                weights[particle] = weights[particle] + weight
                if weight > 0: 
                    weight_zero = False
            ...
            if not weight_zero:
                ...
                for i in range(no_of_particles):
                    sweights = util.sample(weights)
                    self.particles.append(sweights)
            else:
                self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        for x in particles:
            ...
            sample_belief_distr = sample
            updated_beliefs.append(sample_belief_distr)

        self.particles = updated_beliefs
    ...
    def getBeliefDistribution(self):
        ...
        particles = self.particles
        no_of_particles = self.numParticles
        x = 1.0 / no_of_particles #normalizing
        for particle in particles:
            current_belief_distribution[particle] = current_belief_distribution[particle] + x
...
class JointParticleFilter:
        ...
        for eachparticle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None: 
                    eachparticle = self.getParticleWithGhostInJail(eachparticle, i)
                else:
                    distance = util.manhattanDistance(eachparticle[i], pacmanPosition)
                    partial *= emissionModels[i][distance]
            possible[eachparticle] += partial
        if possible.totalCount() == 0:
            self.initializeParticles() 
        else:
            possible.normalize()
            temp = []
            for each in range(0, self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            no_of_ghosts = self.numGhosts
            oldGPositions = list(oldParticle)
            for i in range(no_of_ghosts):
                gs = setGhostPositions(gameState, oldGPositions)
                agent = self.ghostAgents[i]
                newPD = getPositionDistributionForGhost(gs, i, agent)
                newpd = util.sample(newPD)
                newParticle[i] = newpd
    ...
    def getBeliefDistribution(self):
        ...
        count = util.Counter()
        for particle in particles:
            count[particle] = 1 + count[particle]
        count.normalize()
        return count
